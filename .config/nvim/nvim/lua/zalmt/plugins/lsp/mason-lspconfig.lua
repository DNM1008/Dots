return {
	...
	config = function()
		...
		-- Toggle focus
		local diagnostic_focused = false
		local prev_win = nil

		vim.keymap.set("n", "<leader>df", function()
			if not diagnostic_focused then
				-- Save current window
				...
				vim.diagnostic.open_float(nil, {
					...
					scope = "cursor",
					border = "rounded",
					close_events = {}, -- do NOT auto-close while focused
				})

				diagnostic_focused = true
			else
				-- Close diagnostics
				vim.diagnostic.hide(nil, 0)

				-- Restore previous window if it still exists
				if prev_win and vim.api.nvim_win_is_valid(prev_win) then
					vim.api.nvim_set_current_win(prev_win)
				end

				prev_win = nil
				diagnostic_focused = false
			end
		end, { desc = "Toggle focused diagnostics popup" })
