...
if [[ $(< /sys/class/drm/card1-HDMI-A-2/status) = "connected" ]]; then
  xrandr --output eDP-1 --mode 3072 --pos 0x0 --rotate normal --output VGA-1 --off --output HDMI-1 --off --output HDMI-2 --primary --mode 1920x1080 --pos 1920x0 --rotate normal --output DP-1 --off --output HDMI-3 --off --output DP-2 --off --output DP-3 --off
else
  xrandr --auto
fi
...
xwallpaper --output HDMI-2 --zoom ~/.config/qtile/wall1 --output eDP-1 --zoom ~/.config/qtile/wall2 &
